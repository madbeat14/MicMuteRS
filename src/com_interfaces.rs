...
pub unsafe trait IPolicyConfig: windows::core::IUnknown {
    unsafe fn GetMixFormat(&self, pszdevicename: PCWSTR, ppformat: *mut std::ffi::c_void) -> HRESULT;
    unsafe fn GetDeviceFormat(&self, pszdevicename: PCWSTR, bdefault: i32, ppformat: *mut std::ffi::c_void) -> HRESULT;
    unsafe fn ResetDeviceFormat(&self, pszdevicename: PCWSTR) -> HRESULT;
    unsafe fn SetDeviceFormat(&self, pszdevicename: PCWSTR, pendpointformat: *const std::ffi::c_void, pmixformat: *const std::ffi::c_void) -> HRESULT;
    unsafe fn GetProcessingPeriod(&self, pszdevicename: PCWSTR, bdefault: i32, pmftdefaultperiod: *mut i64, pmftminimumperiod: *mut i64) -> HRESULT;
    ...
    unsafe fn GetPropertyValue(&self, pszdevicename: PCWSTR, pkey: *const std::ffi::c_void, pvalue: *mut std::ffi::c_void) -> HRESULT;
    unsafe fn SetPropertyValue(&self, pszdevicename: PCWSTR, pkey: *const std::ffi::c_void, pvalue: *const std::ffi::c_void) -> HRESULT;
...
pub fn set_default_device(device_id: &str) -> bool {
    unsafe {
        ...
        if let Ok(policy_config) = CoCreateInstance::<_, IPolicyConfig>(&CLSID_PolicyConfig, None, CLSCTX_ALL) {
